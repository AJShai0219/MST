...
class Net(torch.nn.Module):  # 继承
    def __init__(self):  # 初始化方法
        ...
        self.fc1 = torch.nn.Linear(28 * 28,
                                   64)  # 这是第一个全连接层（Fully Connected Layer），它将输入的特征维度从 28 * 28（通常是28x28像素的图像展平后的长度）转换为 64 个输出单元。
        ...
        self.fc4 = torch.nn.Linear(64,
                                   10)  # 这是第四个全连接层，将 64 个特征映射到 10 个输出特征，这通常是一个分类任务的输出层，10 代表可能的分类类别数量（例如，对于手写数字分类，可能对应 0-9 的数字）。
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.log_softmax(self.fc4(x),
                                            dim=1)  # log_softmax 是 softmax 的对数版本。这个函数对每个样本的得分进行归一化处理，并计算其对数概率。dim=1 指定在特征维度（即类别维度）上进行操作。
        return x
